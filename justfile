# session-jsonl-compact — task runner
...
demo: build
    @rm -rf demo-out
    ./target/release/session-jsonl-compact -i tests/fixtures/codex_sample.jsonl  -o demo-out
    ./target/release/session-jsonl-compact -i tests/fixtures/claude_sample.jsonl -o demo-out
...
install: build
    ...
    @echo "installed: ~/.local/bin/session-jsonl-compact (ensure ~/.local/bin is on PATH)"
